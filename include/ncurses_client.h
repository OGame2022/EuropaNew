...
static struct dc_application_settings *
create_settings(const struct dc_posix_env *env, struct dc_error *err);
...
size_t get_time_difference(int future_hour, int future_minute, int current_hour,
                           int current_min, int current_seconds);

static void send_game_state(client_info *clientInfo, int udp_socket);

static void receive_udp_packet(const struct dc_posix_env *env,
                               struct dc_error *err, client_info *clientInfo);

static void free_client_entities_list(client_node **head);

static void fill_entities_list(client_node **entities_list,
                               const uint8_t *entity_buffer,
...
static void fill_bullet_list(bullet_node **bullet_list,
                             const uint8_t *entity_buffer,
