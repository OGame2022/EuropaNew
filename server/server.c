...
static void send_game_state(server_info *serverInfo, int udp_socket) {
    ...
    size_t entity_packet_size = 6;
    size_t bullet_packet_size = 4;
    ...
    uint8_t * bullet_list = calloc(num_bullets, bullet_packet_size);
    ...
    memcpy((buffer + header_size + entities_added * entity_packet_size), bullet_list, bullets_added * bullet_packet_size);
    free(header);
    free(entity_list);
    free(bullet_list);
