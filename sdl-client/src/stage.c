...
void draw_players()
{
    ...
    while(node)
    {
        blit(node->client_entity->texture, node->client_entity->position_y * 10, node->client_entity->position_x * 10);
        node = node->next;
    }
}
...
void draw_bullets()
{
    ...
    while (node)
    {
        blit(node->bullet->texture, node->bullet->position_y * 10, node->bullet->position_x * 10);
        node = node->next;
    }
}
